package de.tomsplayground.peanuts.client.controllist;
 ...
 * 
 */
public abstract class ControlListViewer<T> extends StructuredViewer {
...
//			}
					...
					case SWT.TRAVERSE_ARROW_PREVIOUS : {
						...
						if (children.length > 0) {
							...
							for (int i = 0; i < children.length; i++) {
								...
								ControlListItem<T> item = (ControlListItem<T>) children[i];
								if (item.isSelected()) {
									selected = true;
									if (i > 0) {
	...
	public void add(Object[] elements) {
		...
		for (Control element : existingChildren) {
			((ControlListItem<T>) element).dispose();
		}

		for (int i = 0; i < infos.length; i++) {
			ControlListItem<T> item = createNewItem(infos[i]);
			item.updateColors(i);
		}
	 ...
	 */
	private ControlListItem<T> createNewItem(Object element) {
		final ControlListItem<T> item = doCreateItem(control, element);
	...
	protected abstract ControlListItem<T> doCreateItem(Composite parent, Object element);

	@Override
	protected ControlListItem<T> doFindInputItem(Object element) {
		return null;
	}

	@SuppressWarnings("unchecked")
	@Override
	protected ControlListItem<T> doFindItem(Object element) {
		Control[] children = control.getChildren();
		for (Control child : children) {
			...
			if (child.getData().equals(element)) {
				return (ControlListItem<T>) child;
			}
		}
		return null;
	}
	...
	protected List<?> getSelectionFromWidget() {
		...
		for (Control child : children) {
			@SuppressWarnings("unchecked")
			ControlListItem<T> item = (ControlListItem<T>) child;
	...
	protected void internalRefresh(Object element) {
		...
		((ControlListItem<T>) widget).refresh();
	...
	public void remove(Object[] elements) {
		...
		for (int i = 0; i < existingChildren.length; i++) {
			@SuppressWarnings("unchecked")
			ControlListItem<T> item = (ControlListItem<T>) existingChildren[i];
			item.updateColors(i);
		}
		control.layout(true);
		doUpdateContent();
	...
	private void updateVisibleItems() {
		...
		for (Control element : children) {
			@SuppressWarnings("unchecked")
			ControlListItem<T> item = (ControlListItem<T>) element;

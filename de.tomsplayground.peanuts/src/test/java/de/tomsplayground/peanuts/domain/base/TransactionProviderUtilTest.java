...
public class TransactionProviderUtilTest {
	...
	ImmutableList<ITransaction> trList = ImmutableList.<ITransaction>of(
		new BankTransaction(Day.of(2008, 0, 2), BigDecimal.ZERO, ""),
		new BankTransaction(Day.of(2008, 0, 4), BigDecimal.ZERO, ""),
		new BankTransaction(Day.of(2008, 0, 4), BigDecimal.ZERO, ""),
		new BankTransaction(Day.of(2008, 0, 6), BigDecimal.ZERO, ""),
		new BankTransaction(Day.of(2008, 0, 6), BigDecimal.ZERO, ""),
		new BankTransaction(Day.of(2008, 0, 26), BigDecimal.ZERO, ""),
		new BankTransaction(Day.of(2008, 0, 26), BigDecimal.ZERO, ""));

	@Test
	public void emptylist() throws Exception {
		Day from = Day.of(2008, 0, 2);
		Day to = Day.of(2008, 0, 12);
	...
	public void notExistingDayShortDistance() {
		Day from = Day.of(2008, 0, 3);
		Day to = Day.of(2008, 0, 5);
		List<ITransaction> byDate = TransactionProviderUtil.getTransactionsByDate(trList, from, to);
		assertEquals(2, byDate.size());
		assertEquals(Day.of(2008, 0, 4), byDate.get(0).getDay());
		assertEquals(Day.of(2008, 0, 4), byDate.get(1).getDay());
	}

	@Test
	public void notExistingDay() {
		Day from = Day.of(2008, 0, 3);
		Day to = Day.of(2008, 0, 25);
		List<ITransaction> byDate = TransactionProviderUtil.getTransactionsByDate(trList, from, to);
		assertEquals(4, byDate.size());
		assertEquals(Day.of(2008, 0, 4), byDate.get(0).getDay());
		assertEquals(Day.of(2008, 0, 4), byDate.get(1).getDay());
		assertEquals(Day.of(2008, 0, 6), byDate.get(2).getDay());
		assertEquals(Day.of(2008, 0, 6), byDate.get(3).getDay());
	}

	@Test
	public void beforeStartAndBehindEnd() throws Exception {
		Day from = Day.of(2008, 0, 1);
		Day to = Day.of(2008, 0, 30);
		List<ITransaction> byDate = TransactionProviderUtil.getTransactionsByDate(trList, from, to);
		assertEquals(trList.size(), byDate.size());
	}
